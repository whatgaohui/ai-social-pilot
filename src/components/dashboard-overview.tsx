...
function DashboardPipelineOverview() {
  const contentPosts = useContentStore((s) => s.contentPosts);
  const { setRightPanelTab, platform } = useUIStore();
...
export function DashboardOverview() {
  const contentPosts = useContentStore((s) => s.contentPosts);
  const currentPlan = useContentStore((s) => s.currentPlan);
  const knowledgeItems = useContentStore((s) => s.knowledgeItems);
  const persona = usePersonaStore((s) => s.persona);
  const { platform, setRightPanelTab, setLeftPanelTab, isGenerating, setIsGenerating } = useUIStore();
