...
const CATEGORIES: ShortcutCategory[] = [
  { id: "general", label: "通用", icon: Command, color: "bg-violet-500/10 text-violet-600 dark:text-violet-400 border-violet-500/20" },
  { id: "content", label: "内容", icon: FileText, color: "bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-500/20" },
  { id: "ai", label: "AI", icon: Sparkles, color: "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/20" },
  { id: "view", label: "视图", icon: LayoutGrid, color: "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/20" },
  { id: "analytics", label: "分析", icon: BarChart3, color: "bg-rose-500/10 text-rose-600 dark:text-rose-400 border-rose-500/20" },
  { id: "platform", label: "平台", icon: Repeat, color: "bg-rose-500/10 text-rose-600 dark:text-rose-400 border-rose-500/20" },
  { id: "nav", label: "导航", icon: ArrowLeft, color: "bg-sky-500/10 text-sky-600 dark:text-sky-400 border-sky-500/20" },
...
const DEFAULT_SHORTCUTS: ShortcutItem[] = [
  { id: "search", keys: ["⌘", "K"], description: "搜索 / 命令面板", icon: Search, category: "general" },
  { id: "help", keys: ["⌘", "/"], description: "快捷键帮助", icon: Command, category: "general" },
  { id: "settings", keys: ["⌘", ","], description: "设置中心", icon: Settings, category: "general" },
  { id: "escape", keys: ["Esc"], description: "关闭弹窗 / 取消操作", icon: CircleX, category: "general" },
  { id: "new-post", keys: ["⌘", "N"], description: "新建帖子", icon: Plus, category: "content" },
  { id: "save", keys: ["⌘", "S"], description: "保存草稿", icon: Save, category: "content" },
  { id: "ai-generate", keys: ["⌘", "⏎"], description: "AI 生成内容", icon: Sparkles, category: "ai" },
  { id: "ai-optimize", keys: ["⌘", "O"], description: "AI 优化内容", icon: Sparkles, category: "ai" },
  { id: "ai-score", keys: ["⌘", "E"], description: "AI 评分", icon: BarChart3, category: "ai" },
  { id: "panel-persona", keys: ["⌘", "1"], description: "人设面板（知识库）", icon: User, category: "view" },
  { id: "panel-calendar", keys: ["⌘", "2"], description: "日历面板", icon: CalendarDays, category: "view" },
  { id: "panel-workspace", keys: ["⌘", "3"], description: "工作台", icon: FileText, category: "view" },
  { id: "panel-data", keys: ["⌘", "4"], description: "数据面板", icon: BarChart3, category: "analytics" },
  ...
  { id: "month-prev", keys: ["←"], description: "切换月份 / 周", icon: ArrowLeft, category: "calendar", context: "calendar" },
  { id: "month-next", keys: ["→"], description: "切换月份 / 周", icon: ArrowRight, category: "calendar", context: "calendar" },
...
const TOP_SHORTCUTS = DEFAULT_SHORTCUTS.slice(0, 5);
...
function ShortcutRow({
  ...
  const isCustomized = currentKeys.join("+") !== shortcut.keys.join("+");
  ...
  return (
    <div className="flex items-center justify-between py-2 px-3 rounded-lg hover:bg-muted/50 transition-colors group">
      <div className="flex items-center gap-2.5 min-w-0">
        ...
        <span className="text-[13px] text-foreground/80 truncate">{shortcut.description}</span>
        {isCustomized && (
          <Badge variant="outline" className="text-[9px] px-1.5 py-0 h-4 bg-amber-50 text-amber-600 border-amber-200 dark:bg-amber-900/20 dark:border-amber-800 dark:text-amber-400">
            自定义
          </Badge>
        )}
      </div>
...
function KeyRecorderDialog({
  ...
  // Sync recordedKeys and isRecording when dialog opens
  useEffect(() => {
    if (!isOpen) return;
...
function OnboardingTips({ onDismiss }: { onDismiss: () => void }) {
  return (
    <motion.div
      ...
      className="relative max-w-xs p-4 rounded-xl bg-popover border shadow-lg"
    >
      <button
        className="absolute top-2 right-2 h-5 w-5 rounded-full flex items-center justify-center hover:bg-muted transition-colors"
        onClick={onDismiss}
      >
      ...
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="text-xs font-semibold">快捷键提示</span>
      </div>
      <p className="text-[11px] text-muted-foreground mb-3">使用这些快捷键提升操作效率：</p>
      <div className="space-y-1.5">
        {TOP_SHORTCUTS.map((s) => (
          <div key={s.id} className="flex items-center justify-between">
            <span className="text-[11px] text-foreground/70">{s.description}</span>
...
export function FloatingShortcutButton({
  ...
  const [tipsDismissed, setTipsDismissed] = useState(() => {
    ...
    try {
      return localStorage.getItem(SHORTCUT_TIPS_DISMISSED_KEY) === "true";
    } catch {
      return true;
    }
  });
  ...
  const handleDismiss = useCallback(() => {
    ...
    localStorage.setItem(SHORTCUT_TIPS_DISMISSED_KEY, "true");
  }, []);

  return (
    <div className="fab-shortcuts">
      <TooltipProvider delayDuration={200}>
        <Tooltip>
          <TooltipTrigger asChild>
            <button
              className="h-full w-full rounded-full bg-violet-600 hover:bg-violet-700 text-white flex items-center justify-center transition-colors"
              onClick={() => {
                if (!tipsDismissed) {
                  setShowTips(true);
                } else {
      ...
      {/* Onboarding tips popover */}
      <AnimatePresence>
        {showTips && (
          <div className="absolute bottom-12 right-0 z-50">
            <OnboardingTips onDismiss={handleDismiss} />
          </div>
        )}
      </AnimatePresence>
    </div>
...
export function KeyboardShortcutsEnhanced({ open, onOpenChange }: KeyboardShortcutsEnhancedProps) {
    ...
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-[560px] p-0 overflow-hidden gap-0">
          ...
          {/* Category filters */}
          {!searchQuery && (
            <div className="flex gap-1 px-4 pt-2 overflow-x-auto scrollbar-none pb-1">
              <button
                ...
                return (
                  <div key={catId}>
                    ...
                    <div className="flex items-center gap-2 mb-2.5 px-2">
                      ...
                      <span className="text-[10px] text-muted-foreground/60">{shortcuts.length} 个</span>
                    </div>
