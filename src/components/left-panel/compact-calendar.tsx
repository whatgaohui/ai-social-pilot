...
function MoveToDateDialog({ open, onOpenChange, post }: MoveToDateDialogProps) {
  const { updateContentPost } = useContentStore();
...
export function CompactCalendar() {
  const { currentPlan, setCurrentPlan, contentPosts, setContentPosts, updateContentPost, addContentPost, deleteContentPost, knowledgeItems } = useContentStore();
  const { selectedDate, setSelectedDate, isGenerating, setIsGenerating, setSelectedPostId, platform, leftPanelTab } = useUIStore();
  ...
  const handlePreviewAnalytics = useCallback(
    (post: ContentPost) => {
      setSelectedDate(post.scheduledDate);
      setSelectedPostId(post.id);
      const { setRightPanelTab } = useUIStore.getState();
