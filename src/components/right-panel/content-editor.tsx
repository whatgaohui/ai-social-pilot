...
export function ContentEditor({ post, isXHS, onScoreBadgeClick }: ContentEditorProps) {
  ...
  // Streaming state from UI store
  const streamingContent = useUIStore((s) => s.streamingContent);
  const isStreamActive = useUIStore((s) => s.isStreamActive);
            ...
            <div className="flex items-center justify-between">
              {isStreamActive ? (
                <motion.div
                  initial={{ opacity: 0, y: -4 }}
                  animate={{ opacity: 1, y: 0 }}
                  className="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-violet-50 dark:bg-violet-950/30 border border-violet-200 dark:border-violet-800"
                >
                  <Sparkles className="h-3 w-3 text-violet-500 animate-pulse" />
                  <span className="text-[10px] font-medium text-violet-600 dark:text-violet-400">AI正在生成...</span>
                </motion.div>
              ) : post.aiScore > 0 && onScoreBadgeClick ? (
                <ScoreBadge score={post.aiScore} onClick={onScoreBadgeClick} />
              ) : (
              ...
              {(isStreamActive && streamingContent) ? (
                <div className="flex items-center gap-1.5 text-[10px] text-muted-foreground">
                  <span>{streamingContent.length} 字</span>
                </div>
              ...
              {isStreamActive && streamingContent ? (
                <div className="text-sm leading-relaxed whitespace-pre-wrap">
                  ...
                  <motion.span
                    ...
                    className="inline-block w-0.5 h-4 bg-violet-500 ml-0.5 align-text-bottom"
                  />
                </div>
              ) : post.content ? (
