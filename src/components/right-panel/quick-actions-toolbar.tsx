...
interface QuickAction {
  ...
  useStream?: boolean; // Whether this action uses streaming
}
...
export function QuickActionsToolbar() {
  ...
  const handleRegenerate = useCallback(async (): Promise<string> => {
    ...
    setStreamingContent("");
    try {
      const newContent = await streamFetch("/api/ai/generate", {
        type: "auto",
  ...
  }, [selectedPost, platform, persona, knowledgeItems, updateContentPost, streamFetch, setIsStreamActive, setStreamingContent, clearStreaming]);
  ...
  }, [selectedPost, persona, knowledgeItems, platform, updateContentPost, streamFetch, setIsStreamActive, setStreamingContent, clearStreaming]);
            ...
            <div className="relative flex items-center justify-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl bg-background/80 backdrop-blur-xl border border-border/20 shadow-[0_-4px_20px_rgba(0,0,0,0.06)] dark:shadow-[0_-4px_20px_rgba(0,0,0,0.3)] overflow-x-auto scrollbar-none">
                    ...
                    <Tooltip key={action.id}>
                      <TooltipTrigger asChild>
                        <motion.button
                          ...
                          disabled={isLoading || (isStreaming && !isLoading)}
              ...
              {isStreaming && (
                <motion.button
                  ...
                  className="flex items-center justify-center gap-1 h-8 w-8 sm:h-9 sm:w-auto sm:px-2.5 rounded-lg hover:bg-destructive/10 text-muted-foreground hover:text-destructive transition-colors cursor-pointer shrink-0"
                >
                  <StopCircle className="h-4 w-4" />
                  <span className="hidden sm:inline text-[11px] font-medium whitespace-nowrap">停止</span>
                </motion.button>
              )}
            </div>
          </div>
