...
export async function getActiveConfig(): Promise<AIModelConfig | null> {
  ...
  try {
    ...
    if (configs.length > 0) {
      const rawConfig = configs[0] as unknown as AIModelConfig;
      // Decrypt apiKey before caching and using
      const config = {
        ...rawConfig,
        apiKey: decrypt((rawConfig as Record<string, unknown>).apiKey as string || ''),
 ...
 * Returns an object with chatCompletion and chatCompletionStream methods
 */
export async function createAIClient(config?: AIModelConfig | null) {
  ...
  if (!activeConfig) {
    ...
    return {
      ...
      chatCompletionStream: async (messages: Array<{ role: string; content: string }>): Promise<ReadableStream<Uint8Array>> => {
