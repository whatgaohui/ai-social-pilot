...
请用中文回复，条理清晰。`;
  ...
  return result.choices?.[0]?.message?.content || '分析生成失败，请重试。';
}
...
- 标签组合兼顾热度与精准度`;
  ...
  const parsed = extractJSON<PolishedContent>(responseText);
...
- engagementPrediction: 互动预测 (low/medium/high/viral)`;
  ...
  const responseText = result.choices?.[0]?.message?.content || '';
  const parsed = extractJSON<PostAnalysis>(responseText);
...
4. 下一步行动建议`;
  ...
  return result.choices?.[0]?.message?.content || '洞察生成失败，请重试。';
}
