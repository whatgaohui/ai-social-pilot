...
- 建议应涵盖：内容策略、互动优化、发布时间、粉丝增长等不同方面`;
        ...
        if (parsed.recommendations && Array.isArray(parsed.recommendations)) {
          const recommendations: StrategyRecommendation[] = parsed.recommendations.map(
            (rec: Record<string, string>, index: number) => ({
              ...
              priority: ['高', '中', '低'].includes(rec.priority) ? rec.priority : '中',
            })
          );
    ...
    const fallback: StrategyRecommendation[] = [
      { id: `strategy-${Date.now()}-0`, icon: 'TrendingUp', title: '提升内容互动率', description: '增加提问式结尾和互动引导，提升评论区活跃度', priority: '高' },
      { id: `strategy-${Date.now()}-1`, icon: 'Calendar', title: '优化发布时间', description: '晚间19-21点发布笔记，覆盖用户活跃高峰', priority: '中' },
      { id: `strategy-${Date.now()}-2`, icon: 'Sparkles', title: '打造爆款标题', description: '使用数字+emoji+疑问句式，提高点击率', priority: '中' },
      { id: `strategy-${Date.now()}-3`, icon: 'Heart', title: '加强粉丝互动', description: '回复每条评论，建立亲密粉丝关系', priority: '低' },
    ];
