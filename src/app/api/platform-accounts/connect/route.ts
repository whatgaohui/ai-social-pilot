  ...
  try {
    ...
    const updatedAccount = await db.platformAccount.update({
      ...
      data: {
        ...
        // Store credentials based on token type (encrypted)
        ...(tokenType === "api_key" && {
        ...
        ...(tokenType === "cookie" && {
          accessToken: cookie ? encrypt(cookie) : "",
        }),
        ...(platform === "wechat" && tokenType === "api_key" && {
          apiKey: (appId || apiKey) ? encrypt(appId || apiKey) : "",
          apiSecret: (appSecret || apiSecret) ? encrypt(appSecret || apiSecret) : "",
    ...
    const decryptedAccount = {
      ...updatedAccount,
      accessToken: decrypt(updatedAccount.accessToken || ""),
      refreshToken: decrypt(updatedAccount.refreshToken || ""),
      apiKey: decrypt(updatedAccount.apiKey || ""),
      apiSecret: decrypt(updatedAccount.apiSecret || ""),
    };
