...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      if (account.tokenType === "api_key" && account.apiKey && account.apiEndpoint) {
        ...
        if (account.platform === "xiaohongshu") {
          ...
          const res = await fetch(url, {
            ...
            headers: {
              ...
              ...(account.apiKey && { "X-apikey": account.apiKey }),
              ...(account.apiSecret && { "X-apisecret": account.apiSecret }),
            },
          ...
          const tokenUrl = endpoint + "/cgi-bin/token?grant_type=client_credential&appid=" + account.apiKey + "&secret=" + account.apiSecret;
