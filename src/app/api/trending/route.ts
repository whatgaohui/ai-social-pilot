...
export async function GET() {
  try {
    ...
    // Use LLM to analyze search results and generate structured trending topics
      ...
      messages: [
        {
          role: "system",
          content:
            '你是小红书运营专家，擅长追踪平台热点和趋势。请根据搜索结果，生成当前小红书平台的热门话题。返回纯JSON格式，不要包含markdown代码块标记。',
        },
        {
          role: "user",
          content: `根据以下搜索结果，生成8个当前小红书平台的热门话题。
...
- 话题应该涵盖不同领域，贴近真实热点`,
    ...
    // Fallback: return default trending topics
    return NextResponse.json({
      success: true,
      data: getDefaultTrendingTopics(),
    });
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Use LLM to generate detailed content suggestions
    const result = await zai.chat.completions.create({
      model: "glm-4-flash",
      messages: [
        {
...
- tips: 4个实用创作技巧`,
    ...
    const responseText = result.choices?.[0]?.message?.content || "";
    const parsed = extractJSON<ContentSuggestion>(responseText);
    ...
    return NextResponse.json(
      { success: false, error: "生成内容建议失败，请重试" },
      { status: 500 }
    );
  }
}
